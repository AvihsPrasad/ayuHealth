...
import { CameraIcon, InboxIcon, PencilSquareIcon, XCircleIcon, ArrowPathIcon } from '@heroicons/react/24/outline'
  ...
  const handleProfileImgUpload = async () => {
    if (selectedFile && clerkUser) {
      ...
      try {
        ...
        setNotifications((prev: any) => [...prev, { id, message: 'Profile image updated successfully!', type: 'success' }])
        setSelectedFile(null)
      } catch (error) {
        console.error('Failed to update profile image:', error);
        const id = Date.now().toString()
        setNotifications((prev: any) => [...prev, { id, message: 'Failed to update profile image. Please try again.', type: 'error' }])
      } finally {
        setUploading(false)
      }
    }
